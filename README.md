...
**Copy a user's access:** the *Copy access from a user...* button mirrors everything one user has onto another - shared / other-mailbox access (Full Access + Send As), distribution lists, and Teams / Microsoft 365 groups (security groups optional). Enter a *From* and a *To* user, choose what to look for, and click **Preview**. It lists everything it finds as a **checklist, each item labeled with its Exchange type** (shared mailbox, user's mailbox, distribution list, Teams / Microsoft 365 group, security group). **Personal (user) mailboxes come unchecked** - so nothing personal is copied unless you deliberately tick it - while shared mailboxes, lists and groups come checked. Review, tick/untick (there are *All* / *None* shortcuts), then **Copy selected** applies only what's ticked. This matters because a mailbox is classified by its *type*, not how it's used: a personal mailbox that your org uses like a shared one still shows as a user's mailbox, so you decide. Group memberships and Send As are found instantly; Full Access on a **user** mailbox is only discovered when you tick *scan user mailboxes for Full Access* (slower - it scans every mailbox). Dynamic and on-prem-synced groups are listed as skipped since their membership can't be edited here.
