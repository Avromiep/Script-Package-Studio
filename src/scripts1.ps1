	...
	<Border Style="{DynamicResource Card}">
		<StackPanel>
			...
			<Grid Margin="0,12,0,0">
				...
				<!-- Country indicator OUTSIDE the box, to its left: [flag] [+code]. The box holds only the
					 national number (first digit flush-left); a typed country code is lifted up here. -->
				<StackPanel x:Name="PhoneCcGroup" Grid.Column="1" Orientation="Horizontal" VerticalAlignment="Center" Visibility="Collapsed">
					<Border x:Name="FlagPanel" Width="26" Height="18" CornerRadius="3" ClipToBounds="True"
							Background="{DynamicResource PanelBrush}" BorderBrush="{DynamicResource StrokeSoftBrush}" BorderThickness="1">
						<Grid>
							<Image x:Name="FlagImg" Stretch="UniformToFill"/>
							<TextBlock x:Name="FlagChip" FontSize="9" FontWeight="SemiBold" Visibility="Collapsed"
									   Foreground="{DynamicResource TextDimBrush}" HorizontalAlignment="Center" VerticalAlignment="Center"/>
						</Grid>
					</Border>
					<TextBlock x:Name="PhoneCcText" VerticalAlignment="Center" Margin="7,0,9,0" FontWeight="SemiBold"
							   FontSize="13" Foreground="{DynamicResource TextBrush}"/>
				</StackPanel>
				<TextBox x:Name="PhoneInput" Grid.Column="2"/>
			</Grid>
			<TextBlock Text="US/Canada: just the 10 digits. Other countries: type the country code (e.g. +44) - it moves up next to the flag." Style="{DynamicResource Small}" Margin="70,6,0,0" TextWrapping="Wrap"/>
			<StackPanel Orientation="Horizontal" Margin="70,10,0,0">
...
function Add-AuthenticationPhoneMethod {
	...
	function OnAddPhoneButtonClick {
		...
		$phoneNumber = Get-AcPhoneValue $phoneCc $phoneInput   # combines the +code shown by the flag with the national number in the box
	...
	Set-PhoneBannerNew
	# Country indicator (flag + "+code") shown OUTSIDE the box, to its left; the box keeps only the
	# national number. Updated live as you type - US flag/+1 for a bare US number, the right country
	# once a code is typed (the code is then lifted out of the box up next to the flag).
	$phoneCc = [pscustomobject]@{ Group = $scriptForm8.FindName('PhoneCcGroup'); Img = $scriptForm8.FindName('FlagImg'); Chip = $scriptForm8.FindName('FlagChip'); Code = $scriptForm8.FindName('PhoneCcText') }
	Update-AcPhoneField $phoneCc $phoneInput
...
#
# It classifies by the Exchange mailbox TYPE (RecipientTypeDetails), not by how a mailbox is
# used, so it can't tell a personal mailbox that's *used* like a shared one from a truly
# personal one. To handle that honestly: every item is labelled with its type and Preview
# lists them as a checklist - personal (user) mailboxes come UNCHECKED, so nothing personal is
# copied unless you deliberately tick it; shared mailboxes, lists and groups come checked.
...
# Each item is @{ Kind; Id; Name; Type; Personal }. Returns ordered @{ Full; SendAs; Dl; Unified;
    ...
    if ($Opts.Mailbox) {
        ...
        foreach ($mb in $mbxs) {
            ...
            try {
                ...
                if ($perm.Count) {
                    $mrtd = "$($mb.RecipientTypeDetails)"
                    $inv.Full += @{ Kind = 'Full'; Id = "$($mb.PrimarySmtpAddress)"; Name = "$($mb.DisplayName)"; Type = (Get-RecipientTypeName $mrtd); Personal = ($mrtd -eq 'UserMailbox') }
                }
            } catch {}
        }
    }
...
# Apply a FLAT list of chosen items (each @{ Kind; Id; Name }) onto $Target. Each grant tolerates
# its own failure so one hiccup doesn't stop the rest; "already has it" is counted, not failed.
# Returns @{ Granted; Already; Failed = @(string); Total }.
function Copy-UserAccess([string]$Target, [object[]]$Items, [scriptblock]$Progress) {
    $granted = 0; $already = 0; $failed = @()
    $targetId = ''
    if (@($Items | Where-Object { $_.Kind -eq 'Security' }).Count) { try { $targetId = (Get-MgUser -UserId $Target -ErrorAction Stop).Id } catch {} }
    $i = 0; $tot = @($Items).Count
    foreach ($it in $Items) {
        $i++
        if ($Progress) { & $Progress $i $tot }
...
<StackPanel Margin="16" Width="470">
    <Border Style="{DynamicResource Card}">
        <StackPanel>
            <Grid>
            ...
            <TextBlock Text="Look for:" Style="{DynamicResource Small}" Margin="0,12,0,4"/>
...
function Invoke-CopyAccessDialog {
    ...
    function Add-Section([string]$Header, $Items) {
        if (-not @($Items).Count) { return }
        $h = New-Object System.Windows.Controls.TextBlock
        $h.Text = $Header; $h.FontSize = 11; $h.Margin = '0,8,0,2'
        $h.SetResourceReference([System.Windows.Controls.TextBlock]::ForegroundProperty, 'TextDimBrush')
        [void]$itemsHost.Children.Add($h)
        foreach ($x in $Items) {
            $cb = New-Object System.Windows.Controls.CheckBox
            $label = "$($x.Name)"
            if ($x.Type) { $label += "   [$($x.Type)]" }
            if ($x.Id -and $x.Id -ne $x.Name) { $label += "   <$($x.Id)>" }
            $cb.Content = $label
            $cb.IsChecked = (-not $x.Personal)
            $cb.Margin = '2,2,0,0'
            $cb.Tag = $x
            [void]$itemsHost.Children.Add($cb)
        }
    }
    function Set-AllChecks([bool]$On) {
        foreach ($c in $itemsHost.Children) { if ($c -is [System.Windows.Controls.CheckBox]) { $c.IsChecked = $On } }
    }
    function Get-Chosen {
        $chosen = @()
        foreach ($c in $itemsHost.Children) { if ($c -is [System.Windows.Controls.CheckBox] -and $c.IsChecked -and $c.Tag) { $chosen += $c.Tag } }
        return $chosen
    }
    ...
    function OnLoadClick {
        ...
        Add-Section 'Distribution lists' $inv.Dl
        Add-Section 'Teams / Microsoft 365 groups' $inv.Unified
        Add-Section 'Security groups' $inv.Security
        $found = @($inv.Full).Count + @($inv.SendAs).Count + @($inv.Dl).Count + @($inv.Unified).Count + @($inv.Security).Count
        $personal = @(@($inv.Full) + @($inv.SendAs) | Where-Object { $_.Personal }).Count
        $progressBar1.Value = 100
        if (-not $found) {
