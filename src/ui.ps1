...
# Rewrite a phone TextBox to the normalized "+CC national" form so the user SEES the country code +
# space appear (and learns it). Reentrancy-guarded (setting .Text re-fires TextChanged). -OnlyWhenComplete
# is the live-typing mode: it only rewrites a BARE US/Canada number once it's complete (10 digits, or 11
# starting with 1), so it never fights you mid-typing or while you're editing a +CC number; on focus-out
# (no switch) it normalizes whatever it can. A value it can't confidently parse is left exactly as typed.
$script:PhoneFmtBusy = $false
function Set-AcPhoneFormatted($TextBox, [switch]$OnlyWhenComplete) {
	if ($script:PhoneFmtBusy -or -not $TextBox) { return }
	$raw = "$($TextBox.Text)"
	$formatted = Format-PhoneForMfa $raw
	if ($formatted -eq $raw) { return }
	if ($OnlyWhenComplete) {
		$digits = ($raw -replace '[^\d]', '')
		$bareComplete = ($raw.Trim() -notmatch '^\s*(\+|00)') -and ($digits.Length -eq 10 -or ($digits.Length -eq 11 -and $digits[0] -eq '1'))
		if (-not $bareComplete) { return }
	}
	$script:PhoneFmtBusy = $true
	try { $TextBox.Text = $formatted; $TextBox.CaretIndex = $formatted.Length } finally { $script:PhoneFmtBusy = $false }
}

# Calling code -> representative ISO 3166-1 alpha-2 country (for the little flag shown beside the
# 2FA phone box). One code can span several countries; we pick the main one. Bundled flag PNGs live
# in Flags\<iso>.png; a code with no bundled image falls back to a 2-letter chip.
$script:CcToIso = @{ '1'='us'; '7'='ru'; '20'='eg'; '27'='za'; '30'='gr'; '31'='nl'; '32'='be'; '33'='fr'; '34'='es'; '36'='hu'; '39'='it'; '40'='ro'; '41'='ch'; '43'='at'; '44'='gb'; '45'='dk'; '46'='se'; '47'='no'; '48'='pl'; '49'='de'; '51'='pe'; '52'='mx'; '53'='cu'; '54'='ar'; '55'='br'; '56'='cl'; '57'='co'; '58'='ve'; '60'='my'; '61'='au'; '62'='id'; '63'='ph'; '64'='nz'; '65'='sg'; '66'='th'; '81'='jp'; '82'='kr'; '84'='vn'; '86'='cn'; '90'='tr'; '91'='in'; '92'='pk'; '93'='af'; '94'='lk'; '95'='mm'; '98'='ir'; '211'='ss'; '212'='ma'; '213'='dz'; '216'='tn'; '218'='ly'; '220'='gm'; '221'='sn'; '222'='mr'; '223'='ml'; '224'='gn'; '225'='ci'; '226'='bf'; '227'='ne'; '228'='tg'; '229'='bj'; '230'='mu'; '231'='lr'; '232'='sl'; '233'='gh'; '234'='ng'; '235'='td'; '236'='cf'; '237'='cm'; '238'='cv'; '239'='st'; '240'='gq'; '241'='ga'; '242'='cg'; '243'='cd'; '244'='ao'; '245'='gw'; '246'='io'; '248'='sc'; '249'='sd'; '250'='rw'; '251'='et'; '252'='so'; '253'='dj'; '254'='ke'; '255'='tz'; '256'='ug'; '257'='bi'; '258'='mz'; '260'='zm'; '261'='mg'; '262'='re'; '263'='zw'; '264'='na'; '265'='mw'; '266'='ls'; '267'='bw'; '268'='sz'; '269'='km'; '290'='sh'; '291'='er'; '297'='aw'; '298'='fo'; '299'='gl'; '350'='gi'; '351'='pt'; '352'='lu'; '353'='ie'; '354'='is'; '355'='al'; '356'='mt'; '357'='cy'; '358'='fi'; '359'='bg'; '370'='lt'; '371'='lv'; '372'='ee'; '373'='md'; '374'='am'; '375'='by'; '376'='ad'; '377'='mc'; '378'='sm'; '379'='va'; '380'='ua'; '381'='rs'; '382'='me'; '383'='xk'; '385'='hr'; '386'='si'; '387'='ba'; '389'='mk'; '420'='cz'; '421'='sk'; '423'='li'; '500'='fk'; '501'='bz'; '502'='gt'; '503'='sv'; '504'='hn'; '505'='ni'; '506'='cr'; '507'='pa'; '508'='pm'; '509'='ht'; '590'='gp'; '591'='bo'; '592'='gy'; '593'='ec'; '594'='gf'; '595'='py'; '596'='mq'; '597'='sr'; '598'='uy'; '599'='cw'; '670'='tl'; '672'='nf'; '673'='bn'; '674'='nr'; '675'='pg'; '676'='to'; '677'='sb'; '678'='vu'; '679'='fj'; '680'='pw'; '681'='wf'; '682'='ck'; '683'='nu'; '685'='ws'; '686'='ki'; '687'='nc'; '688'='tv'; '689'='pf'; '690'='tk'; '691'='fm'; '692'='mh'; '850'='kp'; '852'='hk'; '853'='mo'; '855'='kh'; '856'='la'; '880'='bd'; '886'='tw'; '960'='mv'; '961'='lb'; '962'='jo'; '963'='sy'; '964'='iq'; '965'='kw'; '966'='sa'; '967'='ye'; '968'='om'; '970'='ps'; '971'='ae'; '972'='il'; '973'='bh'; '974'='qa'; '975'='bt'; '976'='mn'; '977'='np'; '992'='tj'; '993'='tm'; '994'='az'; '995'='ge'; '996'='kg'; '998'='uz' }
$script:FlagCache = @{}
# Make sure every country code we can show a flag for is also recognized by the number parser.
foreach ($k in @($script:CcToIso.Keys)) { [void]$script:CallingCodes.Add($k) }
# The country info for a typed phone number: @{ Cc; Iso; Chip }. Cc/Iso are '' when unknown; Chip is
# the 2-letter fallback text (ISO upper, or "+cc", or '') shown when there's no bundled flag image.
function Get-AcNumberCountry([string]$Raw) {
	$res = [pscustomobject]@{ Cc = ''; Iso = ''; Chip = '' }
	if ([string]::IsNullOrWhiteSpace($Raw)) { return $res }
	$s = "$Raw".Trim()
	$digits = ($s -replace '[^\d]', '')
	if (-not $digits) { return $res }
	$hasCc = $s.StartsWith('+')
	if (-not $hasCc -and $digits.StartsWith('00')) { $hasCc = $true; $digits = $digits.Substring(2) }
	if ($hasCc) {
		$cc = Get-AcCallingCode $digits
		if (-not $cc) { return $res }
	} elseif ($digits.Length -eq 10 -or ($digits.Length -eq 11 -and $digits[0] -eq '1')) {
		$cc = '1'                                   # bare NANP number -> assume US/Canada
	} else {
		return $res                                 # not enough to identify a country yet
	}
	$res.Cc = $cc
	$iso = [string]$script:CcToIso[$cc]
	$res.Iso = $iso
	$res.Chip = if ($iso) { $iso.ToUpper() } else { "+$cc" }
	return $res
}
# Load (and cache) a bundled flag image for an ISO code, or $null if none is bundled. OnLoad+Freeze
# so the file handle is released (same rule as the app icons) and the source is cross-thread safe.
function Get-AcFlagSource([string]$Iso) {
	if (-not $Iso) { return $null }
	$iso = $Iso.ToLower()
	if ($script:FlagCache.ContainsKey($iso)) { return $script:FlagCache[$iso] }
	$src = $null
	try {
		$dir = Join-Path (Split-Path $script:SrcDir -Parent) 'Flags'
		$path = Join-Path $dir "$iso.png"
		if (Test-Path -LiteralPath $path) { $src = New-ImageSource $path }
	} catch { $src = $null }
	$script:FlagCache[$iso] = $src
	return $src
}
# Update a flag host (built by New-FlagHost) to show the country for the given phone text: the flag
# image when one is bundled, else a small 2-letter code chip, else hidden.
function Update-AcFlagHost($FlagHost, [string]$Raw) {
	if (-not $FlagHost) { return }
	$img = $FlagHost.Img; $chip = $FlagHost.Chip; $panel = $FlagHost.Panel
	$info = Get-AcNumberCountry $Raw
	$flag = if ($info.Iso) { Get-AcFlagSource $info.Iso } else { $null }
	if ($flag) {
		$img.Source = $flag; $img.Visibility = 'Visible'; $chip.Visibility = 'Collapsed'
		$panel.Visibility = 'Visible'; $panel.ToolTip = $info.Iso.ToUpper()
	} elseif ($info.Chip) {
		$chip.Text = $info.Chip; $chip.Visibility = 'Visible'; $img.Visibility = 'Collapsed'
		$panel.Visibility = 'Visible'; $panel.ToolTip = $info.Chip
	} else {
